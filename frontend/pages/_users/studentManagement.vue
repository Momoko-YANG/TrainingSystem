    ...
    data() {
      ...
      return {
        ...
        ruleValidate: {
          ...
          phone: [
            { required: true, type: 'integer', min: 1, message: 'Phone is required and must be numeric', trigger: 'change' }
          ],
          birthplace: [
              ...
              h('Button', {
                ...
                on: {
                  click: () => {
                    that.$Modal.confirm({
                      title: 'Confirm',
                      content: `Delete student ${params.row.name}?`,
                      loading: true,
                      onOk: () => {
                        that.$axios({
                          url: '/admin/studentManagement',
                          ...
                          if (res.data.message === 'ok') {
                            that.$Message.info('Student deleted')
    ...
    methods: {
      ...
      async handleAddStudent() {
        ...
        this.$axios({
          ...
          if (res.data.message === 'ok') {
            this.$Message.info(this.isAdding ? 'Student added' : 'Student updated')

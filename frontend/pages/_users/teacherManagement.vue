  ...
  <div class="wrapper">
    ...
    >
      ...
      <Form
        ...
        ref="form"
      >
        <FormItem label="Teacher ID" prop="tid">
        ...
        <FormItem label="Birth Date" prop="birthday">
          <DatePicker type="date" placeholder="Select a birth date" v-model="form.birthday"></DatePicker>
        </FormItem>
  ...
  export default {
    ...
    data() {
      ...
      return {
              ...
              h('Button', {
                ...
                on: {
                  click: () => {
                    that.$Modal.confirm({
                      title: 'Confirm',
                      content: `Delete teacher ${params.row.tname}?`,
                      loading: true,
                      onOk: () => {
                        that.$axios({
                          url: '/admin/teacherManagement',
                          ...
                          if (res.data.message === 'ok') {
                            that.$Message.info('Teacher deleted')
        ...
        ruleValidate: {
          tid: [
            { required: true, message: 'Teacher ID is required', trigger: 'blur' }
          ],
          tname: [
            { required: true, message: 'Name is required', trigger: 'blur' }
          ],
          birthday: [
            { required: true, type: 'date', message: 'Birth date is required', trigger: 'change' }
          ],
          gender: [
            { required: true, message: 'Gender is required', trigger: 'change' }
          ],
          wage: [
            { required: true, type: 'integer', min: 1, message: 'Salary is required and must be positive', trigger: 'change' }
          ],
          education: [
    ...
    methods: {
      ...
      async handleAddTeacher() {
        ...
        this.$axios({
          ...
          if (res.data.message === 'ok') {
            this.$Message.info(this.isAdding ? 'Teacher added' : 'Teacher updated')
